<template>
  <UContainer>
    <UCard>
      <template #header>
        <h2 class="text-xl font-bold">Trip Planner Feedback</h2>
      </template>
      <p class="mb-4">Use this form for questions and constructive comments or suggestions about the Trip Planner.</p>
      <UForm :state="form" :schema="schema" @submit="submit">
        ...
        <UFormField label="Section of Trip Planner" name="section" required>
          <USelect v-model="form.section" :items="sections" class="w-full" />
        </UFormField>
...
const schema = v.object({
  ...
  section: v.pipe(v.string(), v.minLength(1, 'Section is required')),
...
const form = reactive({
  ...
  section: '',
  comments: '',
  caseType: 'Trip Planner',
  files: []
})

onMounted(async () => {
  const data = await $nd.lambda('initCase', { caseType: 'Trip Planner' })
  if (data.payload) {
    commentTypes.value = data.payload.commentTypes || []
    preferences.value = data.payload.preferences || []
    sections.value = data.payload.sections || []
  }
})
