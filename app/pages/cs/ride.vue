<template>
    <UContainer>
        <UCard>
            <template #header>
                <h2 class="text-xl font-bold">Bus Ride Feedback</h2>
            </template>
            <p class="mb-4">Use this form for questions, comments or suggestions about your ride.</p>
...
onMounted(async () => {
    const data = await $nd.lambda('initCase', {caseType: 'Bus Ride'})
    if (data.payload) {
...
const submit = async () => {
    ...
    await $nd.notify('Success', 'Form submitted successfully', {color: 'success'})
